/* eslint-disable max-len */
import React from 'react';

export const IconMore = () => {
  return (
    <svg viewBox='0 0 36 8'  xmlns='http://www.w3.org/2000/svg'>
      <path d='M4 8C6.20914 8 8 6.20914 8 4C8 1.79086 6.20914 0 4 0C1.79086 0 0 1.79086 0 4C0 6.20914 1.79086 8 4 8Z' />
      <path d='M18 8C20.2091 8 22 6.20914 22 4C22 1.79086 20.2091 0 18 0C15.7909 0 14 1.79086 14 4C14 6.20914 15.7909 8 18 8Z' />
      <path d='M32 8C34.2091 8 36 6.20914 36 4C36 1.79086 34.2091 0 32 0C29.7909 0 28 1.79086 28 4C28 6.20914 29.7909 8 32 8Z' />
    </svg>
  );
};

IconMore.iconType = 'SVG-more';
